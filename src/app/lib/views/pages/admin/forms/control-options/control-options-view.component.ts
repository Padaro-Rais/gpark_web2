...
import { Collection, ICollection } from "src/app/lib/core/collections";
      ...
      >
          ...
          <ng-container *ngFor="let name of controlNames; let last = last">
            <clr-dg-cell [class]="last ? 'cell-last' : 'cell'">
              <app-dynamic-inputs
                ...
                *ngIf="typeHelper?.isDefined(formgroup.controls[name])"
                [control]="formgroup.controls[name]"
                [inputConfig]="controlConfigs?.get(name)"
              >
              </app-dynamic-inputs>
              <button
        ...
        <clr-dg-row *ngFor="let item of vm?.source?.data">
          <clr-dg-action-overflow>
            <button class="action-item" (click)="editBtnClicked(item)">
              ÉDITER
            </button>
            <button class="action-item" (click)="deleteBtnClicked(item)">
              SUPPRIMER
            </button>
          </clr-dg-action-overflow>
        ...
        <clr-dg-footer>
          ...
          <clr-dg-pagination
            [clrDgPageSize]="10"
            #pagination
            [clrDgTotalItems]="vm?.source?.total"
          >
            <clr-dg-page-size
...
export class ControlOptionViewComponent implements OnDestroy {
  ...
  @Input() set form(value: IDynamicForm) {
    ...
    if (value) {
      (value.controlConfigs as IHTMLFormControl[]).forEach((config) =>
        this.controlConfigs.add(config?.formControlName, config)
      );
    }
  }
  get form() {
  ...
  controlConfigs: ICollection<IHTMLFormControl> =
    new Collection<IHTMLFormControl>();
  controlNames = [
    "model",
    "table",
    "display_label",
  ...
  gridState$ = this._datagridState$.pipe(
    ...
    switchMap((state: ClrDatagridStateInterface) =>
      this._provider.paginate(state)
    ),
    tap((_) => this._uiState.endAction())
  );

  state$ = combineLatest([this._uiState.uiState, this.gridState$]).pipe(
  ...
  onDgRefresh = (state: ClrDatagridStateInterface = null) =>
    this._datagridState$.next(
      mapPaginatorStateWith([])(state || this.initialGridState)
    );
